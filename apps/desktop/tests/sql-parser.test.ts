...
describe('string literals', () => {
  ...
  it('escaped single quote (doubled apostrophe)', () =>
    expectSplits("SELECT 'it''s fine; still one' FROM t", [
      "SELECT 'it''s fine; still one' FROM t"
    ]))
...
describe('dollar-quoted strings (PL/pgSQL)', () => {
  it('plain $$ quoting', () =>
    expectSplits('CREATE FUNCTION f() RETURNS void AS $$ BEGIN NULL; END; $$ LANGUAGE plpgsql', [
  ...
  it('DO block with multiple internal semicolons', () =>
    expectCount("DO $$ DECLARE x INT; BEGIN x := 1; RAISE NOTICE '%', x; END; $$", 1))
...
describe('DDL statements', () => {
  it('CREATE TABLE', () =>
    expectSplits('CREATE TABLE t (id SERIAL PRIMARY KEY, name TEXT)', [
  ...
  it('ALTER TABLE then SELECT', () =>
    expectSplits('ALTER TABLE t ADD COLUMN x INT; SELECT * FROM t', [
      'ALTER TABLE t ADD COLUMN x INT',
      'SELECT * FROM t'
    ]))

  it('DROP TABLE IF EXISTS', () =>
    expectSplits('DROP TABLE IF EXISTS foo; DROP TABLE IF EXISTS bar', [
      'DROP TABLE IF EXISTS foo',
      'DROP TABLE IF EXISTS bar'
    ]))

  it('CREATE INDEX', () =>
    expectSplits('CREATE INDEX idx_name ON t(name); SELECT 1', [
...
describe('DML sequences', () => {
  it('INSERT then SELECT', () =>
    expectSplits("INSERT INTO t (name) VALUES ('Alice'); SELECT * FROM t", [
  ...
  it('UPDATE then SELECT', () =>
    expectSplits("UPDATE t SET name = 'Bob' WHERE id = 1; SELECT * FROM t WHERE id = 1", [
      "UPDATE t SET name = 'Bob' WHERE id = 1",
      'SELECT * FROM t WHERE id = 1'
    ]))

  it('DELETE then SELECT count', () =>
    expectSplits('DELETE FROM t WHERE id > 100; SELECT COUNT(*) FROM t', [
...
describe('CTEs', () => {
  ...
  it('two separate CTEs', () =>
    expectCount('WITH a AS (SELECT 1) SELECT * FROM a; WITH b AS (SELECT 2) SELECT * FROM b', 2))
...
describe('edge cases', () => {
  it('positional parameters $1 $2 are not dollar-quotes', () => expectCount('SELECT $1, $2', 1))

  it('CREATE TEMP TABLE then SELECT from it', () =>
    expectSplits('CREATE TEMP TABLE tmp AS SELECT id FROM t WHERE active;\nSELECT * FROM tmp', [
      'CREATE TEMP TABLE tmp AS SELECT id FROM t WHERE active',
      'SELECT * FROM tmp'
    ]))

  it('SET session variable then query', () =>
    expectSplits('SET search_path TO myschema; SELECT * FROM t', [
  ...
  it('BEGIN / COMMIT block', () => expectCount('BEGIN; INSERT INTO t VALUES (1); COMMIT', 3))
...
describe('getQueryAtOffset', () => {
  ...
  it('returns the statement containing the cursor', () => {
    expect(getQueryAtOffset(sql, sql.indexOf('users'))).toBe('SELECT *\nFROM users')
  })

  it('distinguishes statements on the same line', () => {
    expect(getQueryAtOffset('SELECT 1; SELECT 2', 18)).toBe('SELECT 2')
  })

  it('uses the preceding statement between statements', () => {
    expect(getQueryAtOffset(sql, sql.indexOf('\n\n') + 1)).toBe('SELECT 1')
  })
})

describe('getQueriesForExecution', () => {
  const editorContent = 'SELECT 1;\nSELECT 2;\nSELECT 3'

  it('runs the selection or current statement supplied by the editor shortcut', () => {
    expect(getQueriesForExecution(editorContent, 'SELECT 2')).toEqual(['SELECT 2'])
  })

  it('runs the full buffer when the toolbar supplies no statement', () => {
    expect(getQueriesForExecution(editorContent)).toEqual(['SELECT 1', 'SELECT 2', 'SELECT 3'])
  })
})
