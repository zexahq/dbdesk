 ...
 *
 * YAML-based persistence for dashboard configurations.
 *
 * Design goals:
 ...
 * - Debounced writes to avoid frequent disk IO during UI interactions
 * - Explicit persist APIs for flush-on-close / flush-on-quit
 */
...
const dashboardCache: Map<string, DashboardConfig> = new Map()
 ...
 * Coalesces many quick updates (e.g. drag/resize) into a single write.
 */
const scheduleFlush = (): void => {
  ...
  flushTimer = setTimeout(() => {
    ...
    void flushCache().catch((error) => {
      console.error('Failed to flush dashboards to disk:', error)
    })
  }, 1000)
}
 ...
 * This updates the in-memory cache and schedules a write to disk
 */
...
export const persistDashboard = async (dashboardId: string): Promise<void> => {
  ...
  // Update the timestamp and mark dirty
