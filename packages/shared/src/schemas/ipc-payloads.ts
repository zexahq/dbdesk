...
const databaseBackupFormatSchema = z.enum(['custom', 'plain', 'tar'])

export const databaseToolChoosePathSchema = connectionIdentifierSchema.extend({
  mode: databaseToolModeSchema,
  format: databaseBackupFormatSchema
})

export const databaseToolRequestSchema = databaseToolChoosePathSchema
  .extend({
    filePath: z.string().trim().min(1).max(4096),
    schemas: z.array(z.string().trim().min(1).max(256)).max(100).optional(),
    tables: z.array(z.string().trim().min(1).max(256)).max(100).optional(),
    clean: z.boolean().optional(),
    verbose: z.boolean().optional(),
    customArgs: z.string().max(4000).optional()
  })
  .superRefine((value, context) => {
    if (
      value.mode === 'restore' &&
      value.format === 'plain' &&
      (value.schemas?.length || value.tables?.length)
    ) {
      context.addIssue({
        code: 'custom',
        path: ['format'],
        message: 'Plain SQL restores do not support schema or table selection'
      })
    }
  })

export const databaseToolStartSchema = databaseToolRequestSchema.and(
  z.object({ jobId: z.string().uuid() })
)

export const databaseToolCancelSchema = z.object({ jobId: z.string().uuid() })
